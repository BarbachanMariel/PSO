...
def generatePop(pop_size):
    ...
    for i in range(pop_size):
        particle = {'RF': np.random.randint(20, 500),
                    'LDA': np.random.uniform(0.01, 0.00001),
                    'SVM': np.random.uniform(0.1, 7.0),
                    'LogReg': [np.random.uniform(0.1, 7.0), np.random.randint(10, 1000)],
                    'KNN': np.random.randint(10, 100),
                   }  # hyperparam
...
def PSO(message, imp, c1, c2, w, max_iter, swarm, pop_size):
    fileBestP = message + imp + "BestParticle5ParticleSwarm.txt"
    out2 = open(fileBestP, "w")

    out2.write(
        "Params:\n" + "c1: " + str(c1) + "\n" + "c2: " + str(c2) + "\n" + "w: " + str(w) + "\n" + "swarm size: " + str(
            pop_size) + "\n" + "iterations: " + str(iter_max) + "\n")
    fileSTR = message + imp + "5ParticleSwarm_trace.csv"
    out = open(fileSTR, "w")
        ...
        for p in swarm:
            ...
            for clf in p[0].keys():
                ...
                if clf == 'LogReg':
                    ...
                    v = w * p[2] + c1 * np.random.uniform(0, 1) * (p[3][clf][0] - p[0][clf][0]) + c2 * np.random.uniform(0,1) * (
                    ...
                    newPosition = abs(p[0][clf] + int(v))
                    if newPosition < 5 or newPosition > len(X_train):
                        print("N neighbours issue")
                        p[0][clf] = np.random.randint(5, 100)
                    else:
                        p[0][clf] = newPosition
                elif clf == 'LDA':  # tol (LDA)
                    v = w * p[2] + c1 * np.random.uniform(0, 1) * (p[3][clf] - p[0][clf]) + c2 * np.random.uniform(0, 1) * (
                                gbest[3][clf] - p[3][clf])
                    p[2] = v

                    newPosition = abs(p[0][clf] + v)
                    if newPosition > .001:
                        print("High tolerance")
                        p[0][clf] = np.random.uniform(0.0001, .01)
                    else:
                        p[0][clf] = newPosition
...
print("reading data...")
MLData = getData("./data/original_PSOdata.csv")
y = MLData.getY()
X = MLData.getX()
...
horizontal_stack.to_csv("Original"+"_validationDataSet.csv", header = True)
...
PSO("PSO", "Unbalanced",c1,c2,w,iter_max, swarm,pop_size)
